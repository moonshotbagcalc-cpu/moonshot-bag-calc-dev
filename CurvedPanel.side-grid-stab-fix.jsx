...
function cpHasSelfCross(pts, eps=0.01){
  ...
  for(let i=0;i<n;i++){
    ...
    if(cpDist(a,b)<eps)continue;
    for(let j=i+2;j<n;j++){
      if(i===0&&j===n-1)continue;
      const c=pts[j],d=pts[(j+1)%n];
      if(cpDist(c,d)<eps)continue;
      if(!cpSegmentsIntersect(a,b,c,d))continue;
      // Ignore touch/cross reports that are effectively the same sampled corner.
...
function cpOffsetInwardMiter(pts,inset){
  const p=cpDedupePath(pts,true);
  const n=p.length;
  if(n<3||!(inset>0))return null;
  const out=[];
  for(let i=0;i<n;i++){
    const prev=p[(i-1+n)%n],cur=p[i],next=p[(i+1)%n];
    if(cpDist(prev,cur)<1e-8||cpDist(cur,next)<1e-8)continue;
    const u1=cpUnit(cur.x-prev.x,cur.y-prev.y);
    const u2=cpUnit(next.x-cur.x,next.y-cur.y);
    const n1={x:-u1.y,y:u1.x};
    const n2={x:-u2.y,y:u2.x};
    let m={x:n1.x+n2.x,y:n1.y+n2.y};
    const ml=Math.hypot(m.x,m.y);
    if(ml<1e-8){
      out.push({x:cur.x+n1.x*inset,y:cur.y+n1.y*inset,side:cur.side});
      continue;
    }
    m={x:m.x/ml,y:m.y/ml};
  ...
  const clean=cpDedupePath(out,true);
...
function cpStabilizerPoints(m,p){
  ...
  // Stabilizer is a clean offset from the CUT edge. Try the same style of
  // per-vertex inward offset used by the core geometry before any fallback.
  const direct=cpOffsetInwardMiter(m.cutPts,inset);
  if(direct?.length>=3&&!cpHasSelfCross(direct,0.006))return direct;

  // Side-aware fallback keeps crisp corner labels/side transitions intact.
  if(m.cutSides?.top&&m.cutSides?.right&&m.cutSides?.bottom&&m.cutSides?.left){
    const sidePaths=cpOffsetSidePaths(m.cutSides,inset);
    const sidePts=cpCombineSidePaths(sidePaths,true);
